...
echo -e "CFLAGS_INCLUDES = -I\$(INCLUDE_DIR) \$(foreach dependencies,\$(DEPENDENCIES_PATH),-I\$(dependencies)/includes)" >> Makefile
...
echo -e "\t@\$(foreach dependencies, \$(DEPENDENCIES_PATH), make -C \$(dependencies);)\n" >> Makefile

echo -e "\$(TARGET): \$(OBJ)" >> Makefile
echo -e "\t\$(CC) \$(CFLAGS) \$(OBJ) \$(foreach dependencies,\$(DEPENDENCIES_PATH),-L\$(dependencies) -l\$(dependencies)) -o \$(TARGET)" >> Makefile
...
echo -e "\t\$(CC) \$(CFLAGS) \$(CFLAGS_DEBUG) \$(OBJ) \$(foreach dependencies,\$(DEPENDENCIES_PATH),-L\$(dependencies) -l\$(dependencies)) -o \$(TARGET_DEBUG)\n" >> Makefile
...
echo -e "\t@\$(foreach dependencies, \$(DEPENDENCIES_PATH), make -C \$(dependencies) clean;)" >> Makefile
echo -e "\t@rm -rf \$(OBJ_DIR)\n" >> Makefile

echo -e "fclean: clean" >> Makefile
echo -e "\t@\$(foreach dependencies, \$(DEPENDENCIES_PATH), make -C \$(dependencies) fclean;)" >> Makefile
echo -e "\t@rm -f \$(TARGET)\n" >> Makefile
